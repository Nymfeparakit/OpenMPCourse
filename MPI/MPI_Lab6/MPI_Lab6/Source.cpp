...
int main(int argc, char* argv[])
{
	...
	const int number_size = 3; //число двоичных разрядов в числе
	...
	if (proc_rank == 0) { //перемножаем последовательно
		...
		cout << "res: " << endl;
		long long res = to_dec(recv_numbers_lint_check[0]);
		cout << res << endl;

	}

	MPI_Comm excl_last_comm;
	MPI_Group world_group;
	MPI_Comm_group(MPI_COMM_WORLD, &world_group);
	MPI_Group group_without_last;
	int rank_to_excl[1]{ proc_num - 1 };
	MPI_Group_excl(world_group, 1, rank_to_excl, &group_without_last);
	MPI_Comm_create(MPI_COMM_WORLD, group_without_last, &excl_last_comm);
	int old_proc_num = proc_num;
	if (proc_num % 2 != 0) {
		--proc_num;
	}
	...
	if (numbers_count >= proc_num) { //если чисел больше, чем процессов
		...
		if (!((proc_num + 1) % 2 != 0 && proc_rank == proc_num)) {
		...
		if (old_proc_num % 2 != 0 && proc_rank != old_proc_num - 1) {
		...
		if (proc_num % 2 != 0) { //если число процессов нечетное
			if (proc_rank != proc_num - 1) {
				MPI_Cart_create(excl_last_comm, 2, dims, periods, reorder, &cart_comm);
			}
			--proc_num;
		}
		else {
		...
		if (! ((proc_num + 1) % 2 != 0 && proc_rank == proc_num)) {
			...
			if (coords[0] == 0) {
				...
				if (column_num_rank == 0) {
					...
					final_arr[0].print();
					cout << "res: " << endl;
					long long res = to_dec(final_arr[0]);
					cout << res << endl;
